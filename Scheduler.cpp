...
static void pickNextMachine(const std::vector<MachineId_t> &vec, unsigned &counter, MachineId_t &outMachine)
{
    if (vec.empty())
        if (counter >= vec.size())
            counter = 0;
    outMachine = vec[counter++];
    if (counter >= vec.size())
        counter = 0;
}
...
void Scheduler::Init()
{
    ...
    for (unsigned int i = 0; i < Machine_GetTotal(); i++)
    {
        ...
        if (t == ARM)
            armMachines.push_back(i);
        else if (t == POWER)
            powerMachines.push_back(i);
        else if (t == RISCV)
            riscvMachines.push_back(i);
        else if (t == X86)
            x86Machines.push_back(i);
    }
...
void Scheduler::NewTask(Time_t now, TaskId_t task_id)
{
    ...
    if (required_cpu == ARM)
        pickNextMachine(armMachines, nextArmMachine, nextMachine);
    else if (required_cpu == POWER)
        pickNextMachine(powerMachines, nextPowerMachine, nextMachine);
    else if (required_cpu == X86)
        pickNextMachine(x86Machines, nextx86Machine, nextMachine);
    else
        pickNextMachine(riscvMachines, nextRiscVMachine, nextMachine);
    ...
    if (minfo.memory_used + task_mem > minfo.memory_size)
    {
        ...
        pending_tasks.push_back(new PendingTask(task_id, nextMachine));
        SimOutput("Scheduler::NewTask(): Insufficient memory on machine " + to_string(nextMachine) + " -> pending task " + to_string(task_id), 2);
        return;
    }
...
void Scheduler::addPendingTasks(Time_t now, MachineId_t machine)
{
    for (auto it = pending_tasks.begin(); it != pending_tasks.end();)
    {
        ...
        if (neededMachine == machine)
        {
            ...
            SimOutput("Scheduler::NewTask(): Task " + to_string(task) + " on Machine " + to_string(neededMachine), 1);
